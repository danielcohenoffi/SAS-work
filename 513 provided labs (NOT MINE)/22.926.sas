...
proc glm data = cracker2 plots = all;
class treat;
model delta = treat;
means treat /hovtest;
output out = diag2 p = yhat r = resid stdr = eresid;
run;

proc sort data = diag2;
by resid;
run;
...
scatter x = last y = resid;
loess x = last y = resid;
refline 0;
run;
...
proc glm data=Cracker;
   class treat;
   model delta=last treat/solution ss3 ;
   lsmeans treat/stderr tdiff pdiff cl;
quit;
...
proc glm data=Cracker;
   class treat;
   model cases=last treat/solution ss3 ;
   lsmeans treat/stderr tdiff pdiff cl;
quit;
