...
proc glm data = flower plots = all;
class group;
model number = group;
means group /hovtest;
output out = diag1 p = yhat cookd = cooks r = resid student = stresid;
run;
...
proc glm data=FLOWER;
   class moisture variety;
   model number=size moisture|variety;
lsmeans moisture*variety;
quit;
...
proc glm data=FLOWER;
   class moisture variety;
   model number=size moisture|variety;
lsmeans moisture*variety/slice=variety;
lsmeans moisture*variety/slice=moisture;
lsmeans  moisture*variety;
