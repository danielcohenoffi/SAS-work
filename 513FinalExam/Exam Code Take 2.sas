...
lsmeans tx*race*gender /pdiff cl adjust = tukey lines;
...
/*satterthwaite*/
proc mixed data = final.data3 plots = all cl maxiter = 1000;
class tx (ref = "1") race gender;
model aae = tx|race|gender/ddfm = satterthwaite s outpm = final.phatfull3;
repeated / group = race*gender*tx;
lsmeans tx*race*gender/cl pdiff;
...
panelby tx /columns = 3;
vline gender /response = estimate group = race markers;
run;
...
proc sgplot data=final.full_lsmeans3;
  by tx;
  series x=gender y=estimate / group=race markers;
  highlow x=gender low=lower high=upper / group=race lowcap=serif highcap=serif;
  yaxis label="Predicted Mean Score";
run;

proc mixed data = final.data3 plots = all cl maxiter = 1000;
class tx(ref = "1") race gender;
model aae = tx race gender tx*race tx*gender race*gender/ddfm = satterthwaite s outpm = final.phatpartial3;
repeated / group = race*gender*tx;
...
model qol = tx|assess /ddfm = satterthwaite;
repeated /group = tx*assess;
run;


proc mixed data = final.data4 covtest plots = none;
class patno tx assess;
model qol = tx|assess / ddfm = satterthwaite outpm = final.phatmixed4untrans residual;
random patno;
repeated assess / subject=patno group = tx*assess r rcorr; 
lsmeans tx /cl pdiff;
...
data ranked;
  set resids;
  adjusted_qol = qol - patient_effect;
  rank = rank(adjusted_qol);
run;


proc mixed data=ranked;
